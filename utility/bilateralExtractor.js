    ...
    res_data.forEach(station_name => {
        if (station_name) {
            ...
            if (station_name === 'Quantum') {
                ...
                // Get the list of equipment objects from the stations
                // remember to filter equipment in the cases where not all is required
                const equipment_to_sum = station_to_add[0]['quantum'].filter( sa => Object.keys(sa)[0] === 'qt1');
                // run logic only if there is an equipment to iterate
                if (equipment_to_sum?.length > 0) {
                    equipment_to_sum.forEach((equip, index) => {
                        // Insert all the first items into the temp hold container, 
            ...
            if (station_name === 'Kam Steel Integrated Ilorin') {
                ...
                // Get the list of equipment objects from the stations
                // remember to filter equipment in the cases where not all is required
                const equipment_to_sum = station_to_add[0]['kamSteel-Ilorin'].filter( sa => Object.keys(sa)[0] === 'l1' || Object.keys(sa)[0] === 'l2');
                // run logic only if there is an equipment to iterate
                if (equipment_to_sum?.length > 0) {
                    equipment_to_sum.forEach((equip, index) => {
                        // Insert all the first items into the temp hold container, 
            ...
            if (station_name === 'ER-KANG Limited') {
                const temp_hold = [];
                const station_to_add = station_array.filter( sa => Object.keys(sa)[0] === 'Er-Kang');
                // Get the list of equipment objects from the stations
                // remember to filter equipment in the cases where not all is required
                const equipment_to_sum = station_to_add[0]['Er-Kang'][0]['Er-Kang'];
                console.log(JSON.stringify(equipment_to_sum), '  the equipment to sum for er-kang');
                // run logic only if there is an equipment to iterate
                if (equipment_to_sum?.length > 0) {
                    equipment_to_sum.forEach((equip, index) => {
                        // Insert all the first items into the temp hold container, 
                        ...
                        if(index == 0 && temp_hold.length == 0) {
                            ...
                            // for the station for this day, any time not here will not be accepted
                            equip[key].forEach( (e) => {
                                temp_hold.push({date: e.date, hour: e.hour, minute: e.minute, kv: e.kv, mw: Math.abs(e.mw), mvar: Math.abs(e.mvar), amp: Math.abs(e.amp), station: 'ER-KANG Limited'})
                            })
                        } else {
                            // Get the key for the next elements
                            const key = Object.keys(equip)[0];
            ...
            if (station_name === 'Ikeja West-Sakete 330kV Line1') {
                ...
                // Get the list of equipment objects from the stations
                // remember to filter equipment in the cases where not all is required
                const equipment_to_sum = station_to_add[0]['ikejaWest-sakate'].filter( sa => Object.keys(sa)[0] === 'l1');
                // run logic only if there is an equipment to iterate
                if (equipment_to_sum?.length > 0) {
                    equipment_to_sum.forEach((equip, index) => {
                        // Insert all the first items into the temp hold container, 
