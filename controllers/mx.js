...
mx.get = (req, res, next) => {
    ...
    const { query } = req;
    const { mx_id, date, level, type } = query;
    ...
    await db.query(model.get, [mx_id, date, level, type])
        .then(resp => {            
            if(resp.rowCount > 0) {
                ...
                db.query(model.create, [date, hour, data, equipment_id, feeder_name, mx_id, station, level, feeder_name, type]);
            }
        })
...
mx.profile = (req, res, next) => {
    // Use the date provided by the app
    const { query } = req;
