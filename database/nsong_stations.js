...
module.exports = (data) => {
    let res_data = {
        ...
        'TAOPEX GS' : {mw: null, kv: null, station: 'TAOPEX GS', amp: null, time: null, seconds: null, mvar:null},
    ...
    station_keys1.forEach(station => {
        ...
        if (filtered_station.length > 0) {
            ...
            if (filtered_station[0].station === 'taopex') {
                let max_voltage = 0, time = filtered_station[0].time, seconds = filtered_station[0].seconds;
                const mw_sum = filtered_station.reduce((acc, curr) => {
                    const sum = acc + Math.abs(curr.mw);
                    max_voltage = max_voltage > curr.kv ? max_voltage : curr.kv;
                ...
                res_data['TAOPEX GS'].mw = mw_sum;
                res_data['TAOPEX GS'].kv = max_voltage;
                res_data['TAOPEX GS'].amp = amp_sum;
                res_data['TAOPEX GS'].mvar = mvar_sum;
                res_data['TAOPEX GS'].time = time;
                res_data['TAOPEX GS'].seconds = seconds;
            }
